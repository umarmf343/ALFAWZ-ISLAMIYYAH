...
describe('AuthProvider', () => {
  ...
  it('attempts refresh before refresh window closes when refresh deadline is sooner', async () => {
    const meta: TokenMeta = {
      expiresAt: new Date(Date.now() + 15 * 60_000).toISOString(),
      refreshExpiresAt: new Date(Date.now() + 30_000).toISOString(),
    };

    api.setToken('initial-token', meta);

    const getSpy = vi.spyOn(api, 'get').mockResolvedValue({ user: baseUser } as any);
    const refreshSpy = vi.spyOn(api, 'refreshAccessToken').mockResolvedValue(true);
  ...
  it('clears authentication immediately when refresh metadata is expired', async () => {
    const meta: TokenMeta = {
      expiresAt: new Date(Date.now() + 15 * 60_000).toISOString(),
      refreshExpiresAt: new Date(Date.now() - 30_000).toISOString(),
    };
